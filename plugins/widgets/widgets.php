...
class wp_widget_page extends WP_Widget
{
    ...
    public function standardization($instance = array())
    {
        $defaults = array(
            'page_id' => null,
            'set_title' => true,
...
<select name="<?php echo $this->get_field_name('page_id'); ?>" >
		<?php foreach ($pages as $page) {
            ?>
<option value="<?php echo $page->ID; ?>" <?php if ($instance['page_id'] == $page->ID) {
...
<p>※ コンテンツエリア（トップページ）への設置推奨</p>
    ...
    public function display_page($pageid, $titleflag = false)
    {
...
<?php foreach ($pages as $page) {
            ?>
<option value="<?php echo $page; ?>" <?php if ($instance['post_type'] == $page) {
                echo 'selected="selected"';
...
<select name="<?php echo $this->get_field_name('display_type'); ?>" id="<?php echo $this->get_field_id('display_type'); ?>">
	<option value="m" <?php if ($instance['display_type'] != 'y') {
            echo 'selected="selected"';
        } ?> >月別</option>
	<option value="y" <?php if ($instance['display_type'] == 'y') {
            echo 'selected="selected"';
        } ?> >年別</option>
</select>
</p>
...
<?php foreach ($taxs as $tax) {
            ?>
	<option value="<?php echo $tax->name; ?>" <?php if ($instance['tax_name'] == $tax->name) {
                echo 'selected="selected"';
...
class WP_Widget_bizvektor_post_list extends WP_Widget
{
    ...
    public function display_pattern_0($posts)
    {
    ...
    public function display_pattern_1($posts)
    {
        global $post;
    ...
    public function display_pattern_2($posts)
    {
        global $post;
        foreach ($posts as $post):
            setup_postdata($post);
    ...
    public function form($instance)
    {
        ...
        $post_types[] = 'post';
        if (!in_array($instance['post_type'], $post_types)) {
...
<?php foreach ($post_types as $posttype): ?>
	<option value="<?php echo $posttype; ?>" <?php if ($instance['post_type'] == $posttype) {
            echo 'selected';
...
<input type="text" id="<?php echo $this->get_field_id('terms'); ?>" name="<?php echo $this->get_field_name('terms'); ?>" value="<?php echo esc_attr($instance['terms']) ?>" /><br />
		...
		<li><label><input type="radio" name="<?php echo $this->get_field_name('format'); ?>" value="0" <?php if ($format == 0) {
            echo 'checked';
        } ?>/>
			<?php echo __('Thumbnail / Title <br>( Sidebar )', 'biz-vektor'); ?></label>
		</li>
		<li><label><input type="radio" name="<?php echo $this->get_field_name('format'); ?>" value="1" <?php if ($format == 1) {
            echo 'checked';
        } ?>/>
			<?php echo __('Date / Category / Title <br>( Main contents area )', 'biz-vektor'); ?></label>
		</li>
		<li><label><input type="radio" name="<?php echo $this->get_field_name('format'); ?>" value="2" <?php if ($format == 2) {
            echo 'checked';
        } ?>/>
