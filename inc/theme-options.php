    ...
    // Info
        ...
        if (is_single()) {
            ...
            if (!empty($taxo_catelist)) :
                $headTitle = get_the_title().' | '.$taxo_catelist.' | '.get_bloginfo('name'); else :
                $headTitle = get_the_title().' | '.get_bloginfo('name');
            endif;
        // Info category
        } elseif (is_tax()) {
...
function biz_vektor_print_headLogo()
{
    ...
    if (isset($options['head_logo']) && $options['head_logo']) {
        echo '<img src="'.$options['head_logo'].'" alt="'.get_bloginfo('name').'" />';
    } else {
        bloginfo('name');
    }
}
...
function biz_vektor_mainfootContact()
{
    ...
    $contact_time = nl2br($options['contact_time']);
    if ($contact_txt) {
        echo '<span class="mainFootCatch">'.$contact_txt.'</span>'."\n";
    }
    if ($options['tel_number']) {
        // mobile
        if (function_exists('wp_is_mobile') && wp_is_mobile()) {
        ...
        if ($contact_time) {
            echo '<span class="mainFootTime">'.$contact_time.'</span>'."\n";
        }
    }
}
...
function biz_vektor_footerSiteName()
{
    ...
    if ($options['foot_logo']) {
        echo '<img src="'.$options['foot_logo'].'" alt="'.$footSiteName.'" />';
    } else {
        echo $footSiteName;
    }
}
    ...
    for ($i = 1; $i <= 5; ++$i) {
        if ($biz_vektor_options['slide'.$i.'image'] && !$biz_vektor_options['slide'.$i.'display']) {
            ...
            if ($biz_vektor_options['slide'.$i.'link']) {
                $blank = '';
                if ($biz_vektor_options['slide'.$i.'blank']) : $blank = ' target="_blank"';
                endif;
