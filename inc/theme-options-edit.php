...
<h3><?php _ex('Design settings', 'biz-vektor theme-options-edit', 'biz-vektor'); ?>
    ...
    foreach ($biz_vektor_theme_styles as $biz_vektor_theme_styleKey => $biz_vektor_theme_styleValues) {
        if ($biz_vektor_theme_styleKey == $options['theme_style']) {
            echo '<option value="'.$biz_vektor_theme_styleKey.'" selected>'.$biz_vektor_theme_styleValues['label'].'</option>';
        } else {
            echo '<option value="'.$biz_vektor_theme_styleKey.'">'.$biz_vektor_theme_styleValues['label'].'</option>';
        }
    } ?>
	</select>
	<?php
    ...
    foreach ($biz_vektor_gMenuDivides as $biz_vektor_gMenuDivideKey => $biz_vektor_gMenuDivideValue) {
        if ($biz_vektor_gMenuDivideKey == $options['gMenuDivide']) {
            echo '<option value="'.$biz_vektor_gMenuDivideKey.'" selected>'.$biz_vektor_gMenuDivideValue.'</option>';
        } else {
            echo '<option value="'.$biz_vektor_gMenuDivideKey.'">'.$biz_vektor_gMenuDivideValue.'</option>';
        }
    } ?>
	</select>
	...
	<?php
        if (!isset($options['side_child_display'])) {
            $options['side_child_display'] = 'side_child_display';
        } ?>
...
<p><input type="checkbox" name="biz_vektor_theme_options[top3PrDisplay]" id="top3PrDisplay" value="true" <?php if ($options['top3PrDisplay']) {
    ?> checked<?php 
} ?>> <?php _e('Do not show the top 3PR area', 'biz-vektor'); ?></p></td>
</tr>
</table>
...
<tr>
	...
	<dd>
	<?php
        if (!isset($options['listBlogTop'])) {
            $options['listBlogTop'] = 'listType_set';
        } ?>
	...
	<dd>
	<?php
        if (!isset($options['listBlogArchive'])) {
            $options['listBlogArchive'] = 'listType_set';
        } ?>
	...
	<dl>
		<?php if (!isset($options['postRelatedCount'])) {
            $options['postRelatedCount'] = 0;
        } ?>
...
<p><input type="checkbox" name="biz_vektor_theme_options[topSideBarDisplay]" id="topSideBarDisplay" value="true" <?php if ($options['topSideBarDisplay']) {
    ?> checked<?php 
} ?>> <?php _e('I want to hide the sidebar', 'biz-vektor'); ?></p></td>
</tr>
...
<td>
<label><input type="checkbox" name="biz_vektor_theme_options[<?php echo $slideDisplay ?>]" id="<?php echo $slideDisplay ?>" value="true" <?php if ($options[$slideDisplay]) : echo ' checked';
        endif; ?>> <?php _ex('Do not display', 'Slide not displayed', 'biz-vektor'); ?></label><br />
<label><input type="checkbox" name="biz_vektor_theme_options[<?php echo $slideBlank ?>]" id="<?php echo $slideBlank ?>" value="true" <?php if ($options[$slideBlank]) : echo ' checked';
        endif; ?>> <?php _e('Open in a blank window', 'biz-vektor'); ?></label>
</td>
</tr>
<?php
    ++$i;
...
<?php
    if (!isset($options['slider_slidespeed']) || !$options['slider_slidespeed']) {
        $options['slider_slidespeed'] = 5000;
    }
    if (!isset($options['slider_animation'])) {
        $options['slider_animation'] = 'fade';
    } ?>
<tr>
