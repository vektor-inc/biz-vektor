...
if(501>a||a<280){jQuery(this).attr("data-width",a);jQuery(this).children("span:first").css({width:a});jQuery(this).children("span iframe.fb_ltr").css({width:a});
}});}fbCommentReSize();jQuery(window).resize(function(){fbCommentReSize();});function fbCommentReSize(){jQuery(".fb-comments").each(function(){var a=jQuery(this).parent().width();
...
if(!this.hash.slice(1)){return;}a=a.length&&a||jQuery("[name="+this.hash.slice(1)+"]");if(a.length){var b=a.offset().top;jQuery("html,body").animate({scrollTop:b},1200,"quart");
return false;}}});});jQuery.easing.quart=function(e,f,a,h,g){return -h*((f=f/g-1)*f*f*f-1)+a;};new function(){var d="footerSection";function a(){var g=document.getElementsByTagName("body")[0].clientHeight;
